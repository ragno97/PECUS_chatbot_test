...
def extract_animals(text: str) -> list[str]:
    """
    Extract explicit animal identifiers only.

    Important: do NOT treat generic counts such as "38 bufale" or
    "146 animali" as animal IDs. V3.1-style evaluation must distinguish
    entity references from herd result counts.
    """
    animals = set()
    ...
    patterns = (
        ...
        # Bare ID allowed only in an ID-like line, e.g. "3118:" or "3118 (L2...)"
        r"^\s*(\d{2,6})\s*(?=[:(\-–—])",
    )
        ...
        return (
            ...
            if (
                ...
                and (
                    ...
                    or re.search(
                        r"\b\d+\s+(?:bufale|vacche|animali|capi)\b",
                        text,
                        flags=re.I,
                    )
                )
...
def primary_answer_text(response: str) -> str:
    """
    Return the answer-bearing part of a PECUS response.

    Scope/entity scoring must not be changed by animals or herd terms
    introduced only in explanations, recommendations or closing offers.
    This restores the V3.1 principle: score the primary answer, not
    incidental recommendation text.
    """
...
def evaluate_scope_and_animal(
    ...
    primary = primary_answer_text(response)
    animals = extract_animals(primary)
    low = primary.lower()
...
def availability_evaluation(row: dict, response: str):
    ...
    if "rumin" in q or "rumin" in profile:
        signal = "rumination"
    elif "grasso" in q or profile == "high_milk_fat":
        signal = "milk_fat"
    elif "conducibil" in q or "conductivity" in profile:
        signal = "conductivity"
    elif "cellule" in q or "scc" in q:
        signal = "scc"
    ...
    explicit_unavailable = bool(re.search(
        r"non (?:è|e) (?:monitorat|disponibil|integrat|presente)|"
        r"non (?:sono|risultano) disponibil[ie]|"
        r"non (?:monitoriamo|monitorate)|"
        r"questa stalla non|"
        r"(?:dato|colonna|parametro).*non (?:è|e) presente|"
        r"non è presente (?:la )?colonna|"
        r"nessuna lettura|"
        r"valori null|"
        r"non ho .* dato",
        low,
    ...
    uncertain = bool(re.search(
        r"se disponibile|se presenti|qualora disponibile",
        low,
        re.I,
    ))

    generic = bool(re.search(
        r"troppo articolat|richiesta complessa",
        low,
        re.I,
    ))

    # Concrete measurement means availability is demonstrated.
    quantitative = False
    if signal == "rumination":
        quantitative = bool(re.search(
            r"\b\d+(?:[.,]\d+)?\s*(?:min|minuti)\b.*rumin|"
            r"rumin\w*.{0,40}\b\d+(?:[.,]\d+)?\s*(?:min|minuti|%)",
            low,
            re.I | re.S,
        ))
    elif signal == "milk_fat":
        quantitative = bool(re.search(
            r"grasso.{0,40}\b\d+(?:[.,]\d+)?\s*%",
            low,
            re.I | re.S,
        ))
    elif signal == "conductivity":
        quantitative = bool(re.search(
            r"conducibil.{0,60}(?:\+|-)?\d+(?:[.,]\d+)?",
            low,
            re.I | re.S,
        ))
    elif signal == "scc":
        quantitative = bool(re.search(
            r"(?:scc|cellule somatiche).{0,60}\d",
            low,
            re.I | re.S,
        ))
    ...
    if quantitative:
        return "PASS", signal, ["SIGNAL_AVAILABILITY_DEMONSTRATED"]
    if uncertain:
        return "REVIEW", signal, ["SIGNAL_AVAILABILITY_UNCLEAR"]
    if generic:
        return "REVIEW", signal, ["SIGNAL_LIMIT_HIDDEN_BEHIND_GENERIC_FALLBACK"]

    # For availability-sensitive domains, an interpretation without either
    # a concrete measurement or an availability statement is not sufficient.
    if signal in {"rumination", "milk_fat"}:
        return "REVIEW", signal, ["SIGNAL_AVAILABILITY_NOT_ESTABLISHED"]
...
def fallback_evaluation(response: str):
    ...
    # A valid empty result is not a data-availability failure.
    zero_result = bool(re.search(
        r"\b0\s+(?:bufale|vacche|animali|capi|risultati)\b|"
        r"nessun[aoei]*\s+(?:bufala|vacca|animale|capo)|"
        r"zero righe per (?:quel|il) criterio|"
        r"non compaiono animali con",
        low,
        re.I,
    ))
    if zero_result:
        return "PASS", ["VALID_ZERO_RESULT"]

    if is_no_data(response):
        clear_reason = bool(re.search(
            r"manca|non disponibil|non monitor|non integrat|"
            r"non (?:è|e) presente|non sono presenti|"
            r"nessuna lettura|valori null|prezzo|"
            r"dataset|colonna|dato",
            low,
            re.I,
        ))
        if clear_reason:
...
def evaluate_attempt(row: dict) -> dict:
    ...
    result.update({
        ...
        "detected_animals": ",".join(extract_animals(primary_answer_text(response))),
        "response_class": response_class(response),
...
def paraphrase_consistency(
    ...
    """
    V3.1-restored paraphrase evaluation.

    - Contextual/elliptical paraphrases are not scored as standalone NLU tests.
    - Herd/group dynamic lists are not required to return the identical
      animal set; live data and answer truncation make raw Jaccard unsuitable
      as a universal gate.
    - Explicit animal paraphrases are scored on intent + entity/context.
    """
    output = []
    grouped = defaultdict(list)
    ...
    for case_id, case_rows in grouped.items():
        case_rows = sorted(
            case_rows,
            key=lambda row: int(row.get("variant_index") or 0),
        )

        canonical = next(
            (row for row in case_rows if row.get("variant_type") == "canonical"),
            case_rows[0],
        )

        canonical_class = canonical["response_class"]
        ...
        for row in case_rows:
            ...
            animal_explicit = as_bool(row.get("animal_explicit", "False"))

            contextual = (
                expected_scope == "animal"
                and expected_animal
                and not animal_explicit
            )

            if contextual:
                status = (
                    "PASS"
                    if row.get("core_status") == "PASS"
                    else row.get("core_status", "REVIEW")
                )
                mode = "contextual"
            else:
                mode = "standalone"

                if row.get("core_status") == "FAIL":
                    status = "FAIL"
                elif row.get("core_status") == "REVIEW":
                    status = "REVIEW"
                elif canonical.get("core_status") == "FAIL":
                    status = "REVIEW"
                else:
                    # For dynamic herd/group lists, semantic equivalence and
                    # correct scope are the gate. Identical IDs are diagnostic,
                    # not mandatory.
                    status = "PASS"
...
def build_aggregate_metrics(rows, consistency):
    ...
    cons_scored=[
        ...
        if c.get("consistency_status")!="BASELINE"
        and c.get("paraphrase_mode")=="standalone"
    ]
    cons_pass=sum(c.get("consistency_status")=="PASS" for c in cons_scored)
    add(
        "Functional Standalone Paraphrase Consistency",
        f"{cons_pass}/{len(cons_scored)}" if cons_scored else "N/A",
        cons_pass,
        len(cons_scored),
        round(100*cons_pass/len(cons_scored),1) if cons_scored else "",
        "functional",
    )

    contextual=[
        c for c in consistency
        if c.get("paraphrase_mode")=="contextual"
    ]
    contextual_pass=sum(c.get("consistency_status")=="PASS" for c in contextual)
    add(
        "Functional Contextual Paraphrase Success",
        f"{contextual_pass}/{len(contextual)}" if contextual else "N/A",
        contextual_pass,
        len(contextual),
        round(100*contextual_pass/len(contextual),1) if contextual else "",
        "functional",
    )
